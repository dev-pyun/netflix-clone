...
class ContentLoader {
  ...
  async loadData() {
    try {
      const response = await fetch('./data/netflix-content.json');
      if (!response.ok) {
  ...
  renderHero() {
    ...
    if (heroImage) heroImage.src = this.data.hero.backgroundImage;
    if (heroLogo) heroLogo.src = this.data.hero.logoImage;
    if (heroDescription) heroDescription.textContent = this.data.hero.description;
  }

  // 슬라이더 재생성
  renderSliders() {
    if (!this.data?.sliders) return;

    const main = document.querySelector('main');
    const hero = document.querySelector('.hero');
    
    // 기존 슬라이더 제거 (히어로 제외)
    const existingSliders = main.querySelectorAll('.slider:not(.slider-hero .slider)');
    existingSliders.forEach(slider => slider.remove());

    // 새 슬라이더들 생성
    this.data.sliders.forEach(sliderData => {
      const sliderElement = this.createSlider(sliderData);
      if (sliderElement) { // null 체크 추가
        main.appendChild(sliderElement);
      }
    });
  }
  ...
  createSlider(sliderData) {
    // 히어로 내부 슬라이더는 기존 HTML 유지
    if (sliderData.type === 'hero') {
      const heroSlider = document.querySelector('.slider-hero .slider');
      if (heroSlider) {
        this.updateHeroSlider(heroSlider, sliderData);
      }
      return null; // 새로 생성하지 않음
    }

    const slider = document.createElement('div');
    slider.className = sliderData.type === 'ranking' ? 'slider top-10-rank-slider' : 'slider';

    slider.innerHTML = `
      <h2 class="slider-title">${sliderData.title}</h2>
      <div class="slider-content">
        ${this.createSliderButton('prev')}
        <ul class="slider-container">
          ${sliderData.items.map(item => this.createSliderItem(item, sliderData.type === 'ranking')).join('')}
        </ul>
        ${this.createSliderButton('next')}
      </div>
    `;
  ...
  // 히어로 슬라이더 업데이트
  updateHeroSlider(heroSlider, sliderData) {
    const container = heroSlider.querySelector('.slider-container');
    const title = heroSlider.querySelector('.slider-title');
    
    if (title) title.textContent = sliderData.title;
    if (container) {
      container.innerHTML = sliderData.items.map(item => this.createSliderItem(item, false)).join('');
    }
  }

  // 슬라이더 버튼 생성
  createSliderButton(type) {
    const direction = type === 'prev' ? 'Left' : 'Right';
    const path = type === 'prev' 
      ? 'M15.5303 3.46967C15.8232 3.76256 15.8232 4.23744 15.5303 4.53033L8.06066 12L15.5303 19.4697C15.8232 19.7626 15.8232 20.2374 15.5303 20.5303C15.2374 20.8232 14.7626 20.8232 14.4697 20.5303L6.46967 12.5303C6.17678 12.2374 6.17678 11.7626 6.46967 11.4697L14.4697 3.46967C14.7626 3.17678 15.2374 3.17678 15.5303 3.46967Z'
      : 'M8.46967 20.5303C8.17678 20.2374 8.17678 19.7626 8.46967 19.4697L15.9393 12L8.46967 4.53033C8.17678 4.23744 8.17678 3.76256 8.46967 3.46967C8.76256 3.17678 9.23744 3.17678 9.53033 3.46967L17.5303 11.4697C17.8232 11.7626 17.8232 12.2374 17.5303 12.5303L9.53033 20.5303C9.23744 20.8232 8.76256 20.8232 8.46967 20.5303Z';

    return `
      <button class="slider-button ${type}-button">
        <svg viewBox="0 0 24 24" width="30" height="30" class="slider-icon">
          <path fill-rule="evenodd" clip-rule="evenodd" d="${path}" fill="currentColor"></path>
        </svg>
      </button>
    `;
  }
  ...
  // 슬라이더 아이템 생성
  createSliderItem(item, isRanking = false) {
    let rankingSvg = '';
    if (isRanking && item.rank) {
  ...
  getFallbackData() {
    return {
      hero: {
        title: "진격의 거인",
        description: "인류의 마지막 거주지인 거대한 성벽 안에서 평화롭게 살아가던 인류는 어느 날 갑작스러운 거인의 출현으로 인해 일상을 잃고 만다.",
        backgroundImage: "../img/attack-on-titan.png",
        logoImage: "../img/banner-attack-on-titan.webp"
      },
      sliders: []
    };
  }
